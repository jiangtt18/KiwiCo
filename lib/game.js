...
class Game {
  constructor(options) {
    ...
    this.players = {
      'red': new HumanPlayer('You'),
      'yellow': new ComputerPlayer()};
    this.turn = this.board.nextDisk();
 ...
 playTurn(){
   while(true){
     ...
     if(this.placeDisk(pos,this.turn)) break;
   }
   this.turn = this.board.nextDisk();
 }
 ...
 drawDisks() {
  for (let row = 0; row < 6; row++) {
      for (let col = 0; col < 7; col++) {
          this.drawDisk(col, row, this.board.board[row][col]); //init with 'white'
      }
  }
}
